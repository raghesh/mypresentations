...
\begin{itemize}
\item Sequential consistency is hard to realize.
\item Too much restriction for hardware and compiler optmizations
\item So we require \textbf{Relaxed Consistency Models}.
\item Same program may exhibit more executions on
      a relaxed model than SC~\cite{Burckhardt2008}.
  \begin{itemize}
  \item Let $T_{\Pi}^Y$ be the set of executions on memory model Y. Then $T_{\Pi}^Y \subset T_{\Pi}^{SC}$.
  \item TODO: Write formally from paper 
  \item Write the two steps to verify a problem in RC.
  \end{itemize}
\end{itemize}
\end{frame}
